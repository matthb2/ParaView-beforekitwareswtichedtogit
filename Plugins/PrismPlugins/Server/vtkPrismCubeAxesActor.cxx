...
vtkCxxRevisionMacro(vtkPrismCubeAxesActor, "$Revision$");
...
void vtkPrismCubeAxesActor::ShallowCopy(vtkPrismCubeAxesActor *actor)
    {
...
void vtkPrismCubeAxesActor::ReleaseGraphicsResources(vtkWindow *win)
    {
    for (int i = 0; i < 4; i++)
        {
...
// Compute the bounds
double *vtkPrismCubeAxesActor::GetBounds()
    {
    return this->Bounds;
    }
...
void vtkPrismCubeAxesActor::PrintSelf(ostream& os, vtkIndent indent)
    {
...
// ****************************************************************************
//  Method: vtkPrismCubeAxesActor::AdjustValues
//
//  Purpose:
...
// ****************************************************************************
//  Method: vtkPrismCubeAxesActor::AdjustRange
//
//  Purpose:
...
int vtkPrismCubeAxesActor::LabelExponent(double min, double max)
    {
    if (min == max)
        {
        return 0;
...
void vtkPrismCubeAxesActor::SetNonDependentAttributes()
    {
...
static int vtkPrismCubeAxesActorTriads[8][3] = {
    ...
    static int vtkPrismCubeAxesActorConn[8][3] = {{1,2,4}, {0,3,5}, {3,0,6}, {2,1,7},
    ...
    // *************************************************************************
    void vtkPrismCubeAxesActor::DetermineRenderAxes(vtkViewport *viewport)
        {
        ...
        if (this->RenderCount++ == 0 || !(this->RenderCount % this->Inertia))
            ...
            if (this->FlyMode == VTK_FLY_CLOSEST_TRIAD)
                ...
                xloc = vtkPrismCubeAxesActorTriads[idx][0];
                yloc = vtkPrismCubeAxesActorTriads[idx][1];
                zloc = vtkPrismCubeAxesActorTriads[idx][2];
                ...
                for (xIdx=0, i=0; i<3; i++)
                    {
                    num = (pts[vtkPrismCubeAxesActorConn[idx][i]][1] - pts[idx][1]);
                    den = (pts[vtkPrismCubeAxesActorConn[idx][i]][0] - pts[idx][0]);
                    if (den != 0.0)
                        {
                        slope = num / den;
                        }
                    if (slope < minSlope && den > 0)
                        {
                        xIdx = vtkPrismCubeAxesActorConn[idx][i];
                        yIdx = vtkPrismCubeAxesActorConn[idx][(i+1)%3];
                        zIdx = vtkPrismCubeAxesActorConn[idx][(i+2)%3];
                        xAxes = i;
                        minSlope = slope;
                        }
                    }
                ...
                zAxes = (xAxes != 0 && yAxes != 0 ? 0 :
                    (xAxes != 1 && yAxes != 1 ? 1 : 2));
                    if (pts[vtkPrismCubeAxesActorConn[xIdx][zAxes]][2] <
                        pts[vtkPrismCubeAxesActorConn[yIdx][zAxes]][2])
                        {
                        zIdx = xIdx;
                        zIdx2 = vtkPrismCubeAxesActorConn[xIdx][zAxes];
                        }
                    else
                        {
                        zIdx = yIdx;
                        zIdx2 = vtkPrismCubeAxesActorConn[yIdx][zAxes];
                        }
                    ...
                    switch (xAxes)
                        {
                        case 0:
                            xloc = vtkPrismCubeAxesActorTriads[mini][0];
                            break;
                        case 1:
                            yloc = vtkPrismCubeAxesActorTriads[mini][1];
                            break;
                        case 2:
                            zloc = vtkPrismCubeAxesActorTriads[mini][2];
                            break;
                        }
    ...
    // *******************************************************************
    // Method: vtkPrismCubeAxesActor::AdjustTicksComputeRange
    //
    ...
    void vtkPrismCubeAxesActor::BuildLabelsFromRanges(vtkAxisActor *axes[4])
        ...
        double extents = inRange[1] - inRange[0];
