...
vtkCxxRevisionMacro(vtkByteSwap, "$Revision$");
...
template <size_t size> class vtkByteSwapSizeType;
static inline void vtkByteSwapper(void*, vtkByteSwapSizeType<1>*)
{
}
static inline void vtkByteSwapper(void* p, vtkByteSwapSizeType<2>*)
{
  char one_byte;
  char* data = static_cast<char*>(p);
  one_byte = data[0]; data[0] = data[1]; data[1] = one_byte;
}
static inline void vtkByteSwapper(void* p, vtkByteSwapSizeType<4>*)
{
  char one_byte;
  char* data = static_cast<char*>(p);
...
static inline void vtkByteSwapper(void* p, vtkByteSwapSizeType<8>*)
{
  char one_byte;
  char* data = static_cast<char*>(p);
...
template <class T> inline void vtkByteSwapRange(T* first, vtkIdType num)
{
  ...
  for(T* p=first; p != last; ++p)
    {
    vtkByteSwapper(p, static_cast<vtkByteSwapSizeType<sizeof(T)>*>(0));
    }
}
...
inline void vtkByteSwapRangeWrite(const T* first, vtkIdType num, FILE* f, long)
{
  ...
  for(const T* p=first; p != last; ++p)
    {
    T temp = *p;
    vtkByteSwapper(&temp, static_cast<vtkByteSwapSizeType<sizeof(T)>*>(0));
