                                             ...
                                             int X, int Y)
{
  ...
  if ( this->State == vtkInteractorStyleTerrain::Rotating ) //left mouse
    {
    float a = (float)(this->Interactor->GetLastEventPosition()[0]-X) / size[0] * 180.0f;
    float e = (float)(this->Interactor->GetLastEventPosition()[1]-Y) / size[1] * 180.0f;
  
    if ( shift )
      {
      if ( fabs((float)(this->Interactor->GetLastEventPosition()[0]-X)) >= fabs((float)(this->Interactor->GetLastEventPosition()[1]-Y)) )
        {
        e = 0.0;
        }
      else
    ...
    this->ComputeDisplayToWorld(double(this->Interactor->GetLastEventPosition()[0]),double(this->Interactor->GetLastEventPosition()[1]), z, p2);
    ...
    {
    double dyf = 10.0 * (double)(Y - this->Interactor->GetLastEventPosition()[1]) /
