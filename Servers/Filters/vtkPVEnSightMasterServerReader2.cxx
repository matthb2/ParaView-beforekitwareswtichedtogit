...
vtkCxxRevisionMacro(vtkPVEnSightMasterServerReader2, "$Revision$");
...
vtkPVEnSightMasterServerReader2::~vtkPVEnSightMasterServerReader2()
{
  ...
  for (rIdx = this->Internal->RealReaders.size() - 1 ; rIdx > 0 ; rIdx--)
    {
  ...
  vtkInformationVector *outputVector)
{
  ...
  if(vtkPVEnSightMasterServerReader2SyncValues(
       ...
       this->Internal->TimeSetValues.size(),
       this->Controller->GetNumberOfProcesses(), this->Controller) != VTK_OK)
    {
  ...
  if ( this->Internal->TimeSetValues.size() > 1 )
    {
    int nTimes = this->Internal->TimeSetValues.size();
    double timeRange[2];
  ...
  vtkInformationVector *outputVector)
{
  ...
  if (outInfo->Has(vtkStreamingDemandDrivenPipeline::UPDATE_TIME_STEPS()) && steps!=0 && tsLength>0 )
    {
...
static int vtkPVEnSightMasterServerReader2GetLineFromStream(
...
int vtkPVEnSightMasterServerReader2::ParseMasterServerFile()
{
  ...
  while(vtkPVEnSightMasterServerReader2GetLineFromStream(fin, line))
    ...
    if(!readingFormat &&
       vtkPVEnSightMasterServerReader2StartsWith(line.c_str(), "FORMAT"))
      {
...
void vtkPVEnSightMasterServerReader2::SetCaseFileName(const char* fileName)
{
  ...
  for (rIdx = this->Internal->RealReaders.size() ; rIdx > 0 ; rIdx--)
    {
    this->Internal->RealReaders[rIdx]->Delete();
    this->Internal->RealReaders.pop_back();
    }
...
{
  return this->Internal->RealReaders.size() == 0 ? 0 : this->Internal->RealReaders[0]->GetNumberOfPointArrays();
}

//----------------------------------------------------------------------------
int vtkPVEnSightMasterServerReader2::GetNumberOfCellArrays()
{
  return this->Internal->RealReaders.size() == 0 ? 0 : this->Internal->RealReaders[0]->GetNumberOfCellArrays();
}

//----------------------------------------------------------------------------
const char* vtkPVEnSightMasterServerReader2::GetPointArrayName(int index)
{
  return this->Internal->RealReaders.size() == 0 ? NULL : this->Internal->RealReaders[0]->GetPointArrayName(index);
}

//----------------------------------------------------------------------------
const char* vtkPVEnSightMasterServerReader2::GetCellArrayName(int index)
{
  return this->Internal->RealReaders.size() == 0 ? NULL : this->Internal->RealReaders[0]->GetCellArrayName(index);
}
...
{
  return this->Internal->RealReaders.size() == 0 ? vtkGenericEnSightReader2::FILE_UNKNOWN_ENDIAN : this->Internal->RealReaders[0]->GetByteOrder();
}

//----------------------------------------------------------------------------
const char *vtkPVEnSightMasterServerReader2::GetByteOrderAsString()
{
  return this->Internal->RealReaders.size() == 0 ? NULL : this->Internal->RealReaders[0]->GetByteOrderAsString();
}
