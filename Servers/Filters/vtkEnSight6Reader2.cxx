...
vtkCxxRevisionMacro(vtkEnSight6Reader2, "$Revision$");
vtkStandardNewMacro(vtkEnSight6Reader2);

//----------------------------------------------------------------------------
vtkEnSight6Reader2::vtkEnSight6Reader2()
{
...
//----------------------------------------------------------------------------
void vtkEnSight6Reader2Read1(const char *line, const char *, int *pointId,
...
void vtkEnSight6Reader2Read2(const char *line, const char *,
                            float *point1, float *point2, float *point3)
{
#ifdef __CYGWIN__  
  // most cygwins are busted in sscanf, this is a work around
...
void vtkEnSight6Reader2Read3(const char *line, const char *,
                            float *point1, float *point2, float *point3,
...
void vtkEnSight6Reader2Read4(const char *line, float *point1)
{
#ifdef __CYGWIN__  
  // most cygwins are busted in sscanf, this is a work around
  int numEntries = 0;
...
//----------------------------------------------------------------------------
int vtkEnSight6Reader2::ReadGeometryFile(const char* fileName, int timeStep,
                                        vtkMultiBlockDataSet *output)
{
  ...
  for (j = 0; j < this->NumberOfUnstructuredPoints; j++)
    ...
    if (pointIdsListed)
      {
      // point ids listed
      vtkEnSight6Reader2Read1(line, " %8d %12e %12e %12e", 
                             &pointId, &point[0],
                             &point[1], &point[2]);
      if (this->UnstructuredNodeIds)
        {
      ...
      {
      vtkEnSight6Reader2Read2(line, "%12e%12e%12e", 
...
//----------------------------------------------------------------------------
int vtkEnSight6Reader2::ReadMeasuredGeometryFile(const char* fileName,
                                                int timeStep,
                                                vtkMultiBlockDataSet *output)
{
  char line[256], subLine[256];
  ...
  for (i = 0; i < this->NumberOfMeasuredPoints; i++)
    {
    this->ReadLine(line);
    vtkEnSight6Reader2Read1(line, " %8d %12e %12e %12e", &tempId, &coords[0],
                           &coords[1], &coords[2]);
...
//----------------------------------------------------------------------------
int vtkEnSight6Reader2::ReadScalarsPerNode(
  ...
  int numberOfComponents, int component)
{
  ...
  if (strncmp(line, "part", 4) != 0)
    ...
    for (i = 0; i < numLines; i++)
      {
      vtkEnSight6Reader2Read3(line, " %12e %12e %12e %12e %12e %12e",
    ...
    for (j = 0; j < moreScalars; j++)
      {
      vtkEnSight6Reader2Read4(line+j*12,&scalarsRead[j]);
      scalars->InsertComponent(i*6 + j, component, scalarsRead[j]);
      }
    if (moreScalars != 0)
      {
...
//----------------------------------------------------------------------------
int vtkEnSight6Reader2::ReadVectorsPerNode(
  const char* fileName, const char* description, int timeStep,
  vtkMultiBlockDataSet *compositeOutput, int measured)
{
  char line[256];
  ...
  if (strncmp(line, "part", 4) != 0)
    ...
    for (j = 0; j < moreVectors; j++)
      {
      vtkEnSight6Reader2Read4(line+j*36,&vector1[0]);
      vtkEnSight6Reader2Read4(line+j*36+12,&vector1[1]);
      vtkEnSight6Reader2Read4(line+j*36+24,&vector1[2]);
  ...
  while (strncmp(line, "part", 4) == 0)
    ...
    for (k = 0; k < 3; k++)
      ...
      if (moreVectors)
        ...
        for (j = 0; j < moreVectors; j++)
          {
          vtkEnSight6Reader2Read4(line+j*12,&values[j]);
          vectors->InsertComponent(i*6 + j, k, values[j]);
          }
        }
      }
...
//----------------------------------------------------------------------------
int vtkEnSight6Reader2::ReadTensorsPerNode(
  const char* fileName, const char* description, int timeStep,
...
//----------------------------------------------------------------------------
int vtkEnSight6Reader2::ReadScalarsPerElement(
  const char* fileName, const char* description, int timeStep,
...
//----------------------------------------------------------------------------
int vtkEnSight6Reader2::ReadVectorsPerElement(
  const char* fileName, const char* description, int timeStep,
  vtkMultiBlockDataSet *compositeOutput)
{
  char line[256];
...
//----------------------------------------------------------------------------
int vtkEnSight6Reader2::ReadTensorsPerElement(
  const char* fileName, const char* description, int timeStep,
  vtkMultiBlockDataSet *compositeOutput)
{
  char line[256];
...
//----------------------------------------------------------------------------
int vtkEnSight6Reader2::CreateUnstructuredGridOutput(
...
//----------------------------------------------------------------------------
int vtkEnSight6Reader2::CreateStructuredGridOutput(
  int partId, char line[256], const char* name,
  vtkMultiBlockDataSet *compositeOutput)
{
  char subLine[256];
  ...
  if (moreCoords != 0)
    ...
    for (j = 0; j < moreCoords; j++)
      {
      vtkEnSight6Reader2Read4(line+j*12,&coords[j]);
      points->InsertNextPoint(coords[j], 0.0, 0.0);
      }
    }
  for (i = 0; i < numLines; i++)
...
//----------------------------------------------------------------------------
void vtkEnSight6Reader2::PrintSelf(ostream& os, vtkIndent indent)
{
